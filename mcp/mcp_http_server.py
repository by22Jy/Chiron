...
async def handle_news_tool(request: ToolRequest) -> Dict[str, Any]:
    ...
    try:
        ...
        news_list = await news_service.get_top_news(count)
...
async def execute_news_weather_email_workflow(context: Dict[str, Any]) -> Dict[str, Any]:
    ...
    try:
        ...
        # 1. 获取新闻
        news_list = await news_service.get_top_news(10)
...
📰 头条新闻 (前10条):
{chr(10).join([f'{i+1}. {news["title"]}' for i, news in enumerate(news_list[:10])])}
